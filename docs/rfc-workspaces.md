...
- **Every org gets a builtin "General" team workspace** (id `t-shared`) provisioned on its first touch — fresh orgs and migrated ones share the same shape: Personal + General. Its `builtin: org` registry row makes every org member an editor with no per-user ACL rows, so membership stays dynamic. Personal remains the default active workspace (privacy by default). An org admin deleting General is permanent — provisioning is gated by the migration marker (versioned `v: 2`; v1-marked orgs get a one-time provisioning upgrade). Solo users get no General.
- **Existing deployments** migrate lazily, once per org, on the org's first request after the flag flips (`state.ensure_migrated`): everything under the org root — files, `.db`, `.database` — moves into General; legacy chats stay coherent because their attachments move with them. Solo users migrate into their personal workspace instead. gcsfuse moves drop GCS custom metadata, so the migration rewrites it from the canonical bodies afterward (`_restore_meta`), and the chat/share listings self-heal any row whose meta channel is missing. The `migrated` marker row in `.org/` gates re-runs across restarts and an `asyncio.Lock` serializes concurrent first requests within an instance; cross-instance races are benign (each move is idempotent) but flip the flag during low traffic anyway. Old share links without `?ws=` fall back to the owner's personal workspace, then `t-shared`.
