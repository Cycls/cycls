...
| `connector_connect_clicked` / `connector_dismissed` / `connector_disconnected` | a connector was opened for connecting (`source` = `card` or `directory`), declined from the card, or disconnected (+ `connector`) | clicked ÷ `ui_action{connect}` is the connect rate; a high dismiss rate on one connector says its card isn't earning trust |
