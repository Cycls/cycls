...
**Status**: built (2026-09-23) — `77e10f9`, `da49a52`, `3fc7d78`, `c615050`.
...
| clear (tier 1) | past the trigger | no | `Session.clear`, `compact.clear` |
...
| clearing must free | 10% of window | 100k | 12.8k |
...
It runs only if it frees at least 10% of the window. Each clear changes the prompt
prefix and costs one cache miss, so it happens in batches, not every turn. There is no
model call, no UI and no wait. This is the approach of Anthropic's tool-result clearing
(`clear_at_least`, keep the recent uses) and OpenHands' observation masking.
...
Failures are `level=warn` rows whose `message` starts with `compaction failed:`; their
`error_id` is the reference the user saw. Tier 1 logs nothing — read `cleared` in the
chat's `compaction.json` (`cycls volume get`).

## Tests

- `tests/agent/agent_test.py`, compaction section — trigger and threshold, tier 1 (stubs,
  no summary call, marker, transcript intact), end-of-run summary, failed and empty
  summaries, cut invariants, ledger, both overflow paths, the `read` cap.
