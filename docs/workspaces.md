...
registry row and nothing else. Legacy content under the org root stays where
...
- `AGENT.md` and `skills/` are per-workspace, so in a team any editor's
  instructions run in teammates' sessions. That is the same trust boundary as
  a shared repo, and worth knowing.
