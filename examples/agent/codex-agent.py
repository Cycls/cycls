...
async def rpc_write(proc, **msg):
    proc.stdin.write((json.dumps({k: v for k, v in msg.items() if v is not None}) + "\n").encode())
    await proc.stdin.drain()


async def rpc_read(proc, expected_id, res, stop=None):
...
async def rpc_call(proc, s, mid, method, params=None, stop=None):
    await rpc_write(proc, id=mid, method=method, params=params)
    s["_res"] = {}
    async for notif in rpc_read(proc, mid, s["_res"], stop):
...
async def handle(proc, notif, s):
    if "id" in notif and "method" in notif:
        ...
        if method == "item/tool/call":
            ...
            await rpc_write(proc, id=notif["id"], result={"contentItems": [{"type": "inputText", "text": f"{tool} rendered successfully"}], "success": True})
        elif "requestApproval" in method:
            p = notif.get("params", {})
            cmd = (p.get("commandActions") or [{}])[0].get("command") or parse_cmd(p.get("command", "")) or json.dumps(p)
            desc = f"\n**Bash(** {cmd} **)**\n"
            if cwd := p.get("cwd"):
                desc += f"dir: `{cwd}`\n"
            if reason := p.get("reason"):
                desc += f"reason: {reason}\n"
            await rpc_write(proc, id=notif["id"], result={"decision": "decline"})
...
async def Agent(ws, home, prompt, policy="never", session_id=None, tools=None, model=None, effort=None, instructions=None, pending=None):
    ...
    try:
        ...
        if not s["_res"]:
            ...
            return
        mid += 1
        await rpc_write(proc, method="initialized")
        ...
        async for out in rpc_call(proc, s, mid, "thread/resume" if session_id else "thread/start", thread_params):
            yield out
        ...
        async for out in rpc_call(proc, s, mid, "turn/start", turn_params, stop=lambda: s["done"] or s["approval"]):
            yield out
        mid += 1
...
@cycls.app(
    apt=["curl", "xz-utils"], copy=[".env"], # proot
    run_commands=[
