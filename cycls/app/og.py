...
def _prep(text):
    """Reshape Arabic text for proper joined rendering."""
    if not _rtl(text):
        return text
    try:
...
def _font(text, bold=False, size=48):
    key = "arabic" if _rtl(text) else "inter"
    path = _FONTS[key][0 if bold else 1]
    try:
...
def _truncate(text, font, max_w):
    if not text or font.getlength(text) <= max_w:
        return text or ""
...
def generate(title, desc="", avatars=None):
    ...
    title = _prep(title)
    desc = _prep(desc) if desc else ""

    tf = _font(title, bold=True, size=78)
    df = _font(desc or title, size=42)

    # Title
    tt = _truncate(title, tf, max_w)
