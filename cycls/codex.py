"""Codex app-server JSON-RPC protocol wrapper.

Thin runtime that manages the codex subprocess lifecycle, streams
notifications, and dispatches tool calls. Zero built-in tools —
tools are passed in by the caller.
"""
...
class Codex:
    ...
    Tools are passed in — this class has no built-in tool definitions.
    Tools with a ``"handler"`` key are dispatched automatically;
    tools without one yield ``tool_call`` events for the caller to handle
    via :meth:`respond`.
    """
    ...
    def _tool_handler(self, name):
        """Return the handler for a named tool, or None."""
        for t in self.tools:
            if t.get("name") == name and "handler" in t:
                return t["handler"]
        return None
    ...
    async def _handle(self, proc, notif, s):
        if "id" in notif and "method" in notif:
            ...
            if method == "item/tool/call":
                ...
                handler = self._tool_handler(tool)
                if handler is not None:
                    events, response_text = await handler(args)
                    for ev in events:
                        yield ev
                    ...
                    # No handler — yield tool_call for caller to handle via respond()
                    yield {"type": "tool_call", "id": notif["id"], "tool": tool, "args": args}
                return
    ...
    async def respond(self, tool_call_id, result):
        """Send a tool response back to the codex process.

        Use this for tools that lack a ``"handler"`` key — the caller
        receives a ``tool_call`` event, does its own work, then calls
        ``respond()`` with the result text.
        """
    ...
    async def run(self, context, *, workspace=None):
        ...
        # Build dynamicTools list — strip "handler" keys before sending to codex
        dynamic_tools = [{k: v for k, v in t.items() if k != "handler"} for t in self.tools]
