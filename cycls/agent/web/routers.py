...
def chats_router(required_auth, volume, bucket):
    r = APIRouter()

    @r.get("/chats")
    async def list_chats(user: Any = required_auth):
        ws = _ws(user, volume, bucket)
        items = []
...
def files_router(required_auth, volume, bucket):
    r = APIRouter()
    ...
    async def list_files(request: Request, user: Any = required_auth):
        target = _safe_path(_ws(user, volume, bucket).root, request.query_params.get("path", ""))
    ...
    async def get_file(path: str, request: Request, user: Any = required_auth):
        file_path = _safe_path(_ws(user, volume, bucket).root, path)
    ...
    async def rename(path: str, request: Request, user: Any = required_auth):
        src = _safe_path(_ws(user, volume, bucket).root, path)
        if not src.exists():
            raise HTTPException(status_code=404, detail="Not found")
        data = await request.json()
        dest = _safe_path(_ws(user, volume, bucket).root, data["to"])
    ...
    async def mkdir(path: str, user: Any = required_auth):
        dir_path = _safe_path(_ws(user, volume, bucket).root, path)
        dir_path.mkdir(parents=True, exist_ok=True)
        return {"ok": True}

    @r.delete("/files/{path:path}")
    async def delete_path(path: str, user: Any = required_auth):
        target = _safe_path(_ws(user, volume, bucket).root, path)
...
def share_router(required_auth, volume, bucket):
    r = APIRouter()
    ...
    async def list_shares(user: Any = required_auth):
        items = []
        async for sid, meta in share.list_shares(_ws(user, volume, bucket)):
            items.append({
                "id": meta.get("id", sid),
...
def install_routers(app, required_auth, volume, bucket):
    ...
    app.include_router(chats_router(required_auth, volume, bucket))
    app.include_router(files_router(required_auth, volume, bucket))
    app.include_router(share_router(required_auth, volume, bucket))
