...
class AppleIAP:
    ...
    header — proves the request comes from an active subscriber.
    """
    def __init__(self, bundle_id, products, namespace, plan="u:iap",
                 header="x-apple-entitlement", root_cert: bytes | None = None):
        self.bundle_id = bundle_id
        self.products = dict(products) if isinstance(products, dict) else {p: plan for p in products}
    ...
    def verify(self, jws, user_id) -> Optional[dict]:
        """The verified transaction payload iff *jws* is a valid, unexpired,
        unrevoked transaction for one of our products, purchased by *user_id*;
        None otherwise. Never raises."""
        try:
            t = self._decode(jws)
            return t if (t.get("bundleId") == self.bundle_id
                         ...
                         and t.get("revocationDate") is None) else None
        except Exception:
            return None
    ...
    def apply(self, user, headers):
        ...
        if user.plan and not user.plan.startswith("u:"):
            return
        if t := self.verify(jws, user.id):
            user.plan = self.products[t["productId"]]
